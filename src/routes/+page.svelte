<script>
  ...
  function defaultPick() {
    ...
    names = Array.from(byName.entries())
      .sort((a, b) => b[1] - a[1])
      .map((d) => d[0]);
  ...
  function seriesCounts(name) {
    const map = new Map(
      rows.filter((r) => r.name === name).map((r) => [r.year, r.count]),
    );
  ...
  function sparkPath(values, w = 140, h = 54, pad = 6) {
    ...
    const xs = values.map(
      (_, i) => pad + (i * (w - 2 * pad)) / (values.length - 1 || 1),
    );
  ...
  function drawLine() {
    ...
    // ----- TOTAL LINE (>=2015 only) -----
    if (showTotal && years.length && totals.size) {
      const filtered = years
        .filter((y) => y >= 2015) // only data from 2015
        .map((y) => ({ y, val: totals.get(y) ?? 0 }));

      traces.push({
        x: filtered.map((d) => d.y),
    ...
    // ----- INDIVIDUAL NAMES (>=2015 only) -----
    for (const nm of pick) {
      const yvals = seriesCounts(nm);

      const filtered = years
        .map((y, i) => ({ y, val: yvals[i] }))
        .filter((d) => d.y >= 2015);
    ...
    // ----- Crown + YoY annotations (>=2015 only) -----
    const crownYear = cursorYear ?? barYear;
    if (crownYear != null && crownYear >= 2015) {
    ...
    Plotly.newPlot(
      ...
      traces,
      {
        xaxis: { title: "Year", range: [2014, years.at(-1)] }, // axis starts 2014
  ...
  function drawBars() {
    ...
    Plotly.newPlot(
      ...
      {
        xaxis: { title: "Licenses (count)", rangemode: "tozero" },
  ...
  function tidyCleaned(arr) {
    ...
    for (const r of arr) {
      ...
      for (const key of ["year", "extract year", "extract_year"]) {
        if (lower[key] != null) {
          const m = String(lower[key]).match(/\d{4}/);
          if (m) {
            year = +m[0];
            break;
          }
      ...
      // NAME
      let name = "";
      const nameKeysPriority = ["animal name", "pet name", "dog name", "name"];
      for (const key of nameKeysPriority) {
        if (lower[key] != null) {
          name = String(lower[key]).trim();
          break;
        }
      }
      if (!name) {
      ...
      for (const key of breedKeys) {
        if (lower[key] != null) {
          breed = String(lower[key]).trim().toUpperCase();
          break;
        }
      }
...
onMount(async ()=>{
  ...
  function drawZipTreemap() {
    ...
    Plotly.newPlot(
      ...
      {
        title: zipOverall
          ? "Where are the dogs? (NYC → Borough → ZIP)"
  ...
  function drawBreedChart() {
    ...
    function colorFor(breed, gender) {
      let hash = 0;
      for (let i = 0; i < breed.length; i++) {
        hash = (hash * 31 + breed.charCodeAt(i)) % 360;
      }
      const baseHue = hash; // stable hue based on breed name
      const lightness = gender === "M" ? 55 : 65;
    ...
    for (const breed of topBreeds) {
      for (const gender of ["M", "F"]) {
        ...
        datasets.push({
          ...
          data,
          backgroundColor: colorFor(breed, gender),
          stack: breed,
        });
      }
    }
...
{:else}
  <div class="container">
    ...
    <section class="card" style="margin-top:16px;">
      ...
      <div class="flex" style="align-items:center;">
        <label style="display:flex;gap:8px;align-items:center;">
          <input
            type="checkbox"
            bind:checked={zipOverall}
            on:change={drawZipTreemap}
          />
    ...
    <section class="card" style="margin-top:16px;">
      ...
      {#if barYear}
        {#if topKForYear(barYear, 3).length}
          {#key barYear}
            <div class="podium">
              {#each topKForYear(barYear, 3) as d (d.name)}
                <div
                  class={`place r${d.rank}`}
                  style={`--c:${colorFor(d.name)}`}
                >
    ...
    <section class="card" style="margin-top:12px;">
      ...
      <div
        class="card"
        bind:this={barsEl}
        style="height:520px;margin-top:10px;"
      ></div>
    </section>
  </div>
{/if}
...
<style>
  ...
  .place::after {
    ...
    background: linear-gradient(
      180deg,
      color-mix(in oklab, var(--c) 12%, #fff) 0%,
      #fff 60%
    );
    opacity: 0.55;
    pointer-events: none;
