<script>
  ...
  // story beats (visual-only chips)
  const steps = [
    { icon: '🐶', pick: ['BELLA','MAX','CHARLIE'], year: null },
    { icon: '🌙', pick: ['LUNA','BELLA','MAX'],    year: 2019 },
    { icon: '🩺', pick: ['LUNA','BELLA','MAX','CHARLIE'], year: 2020 },
    { icon: '🔁', pick: ['LUNA','BELLA','COCO'],   year: 2023 }
  ];
  ...
  // build minimal sparkline path for values (normalized to w×h)
  ...
  function drawLine(){
    ...
    for (const nm of pick){
      ...
      traces.push({
        ...
        name: nm, line:{color:colorFor(nm)}, marker:{color:colorFor(nm)},
    ...
    const shapes = [
      { // pandemic band
        type:'rect', xref:'x', yref:'paper', x0:2020, x1:2021, y0:0, y1:1,
        fillcolor:'rgba(0,0,0,0.05)', line:{width:0}
      }
    ];
    if (cursorYear!=null){
      shapes.push({
        type:'line', xref:'x', yref:'paper', x0:cursorYear, x1:cursorYear, y0:0, y1:1,
    ...
    if (crownYear!=null){
      ...
      if (best){
        const t = traces.find(t=>t.name===best.name);
        if (t){
          const xi = years.indexOf(crownYear);
          const yv = t.y[xi] ?? 0;
          ann.push({ x:crownYear, y:yv, xref:'x', yref:'y', text:'👑', showarrow:false, font:{size:22} });
        }
      }
    }
...
<div class="container">
  ...
  <!-- Sticky trend + scrolly steps (visual-only chips) -->
    ...
    <div class="steps">
      {#each steps as s}
        <article class="step card"
          use:inView={{ onEnter: () => { pick = s.pick; sceneYear = s.year ?? null; cursorYear = s.year ?? null; drawLine(); } }}>
          <div class="row">
  ...
  <!-- YEAR LEADERBOARD: podium + background sparklines + crying bench -->
...
<style>
  ...
  .chip{ background:#f2f4f7; border:1px solid #e5e7eb; border-radius:999px; padding:2px 10px; font-size:12px; letter-spacing:.2px; color:var(--c); }
